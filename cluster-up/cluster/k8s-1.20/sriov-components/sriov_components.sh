...
KUBEVIRTCI_CONFIG_PATH="$kubevirtci_path/_ci-configs"

MANIFESTS_DIR="${kubevirtci_path}/cluster-up/cluster/${KUBEVIRT_PROVIDER}/sriov-components/manifests"
...
function sriov_components::deploy() {
  set -x
  local -r drivers=$1
  local -r resource_prefix=$2
  local -r resource_name=$3
  local -r label_key=$4
  local -r label_value=$5
